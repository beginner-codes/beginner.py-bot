...
class ModuleWrapper(bevy.Bevy):
    ...
    def __init__(self, module: ModuleType):
        self.__protected_module__ = module  # Use dunder name so that our getattr code will protect it
        self.__enabled_attributes = self.config.get("enabled_modules").get(self.__protected_module__.__name__, [])

        if not self.__enabled_attributes:
            raise RunnerImportError(f"The module '{self.__protected_module__.__name__}' is disabled for security reasons")
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if not self.__enabled_attribute(name):
            raise RunnerAttributeError(f"The attribute or method '{self.__protected_module__.__name__}.{name}' is disabled for security reasons")
    ...
    def __setattr__(self, name: str, value: Any):
        if not hasattr(self, "__protected_module__") or name.startswith(f"_{self.__class__.__name__}__") or name.startswith("__"):

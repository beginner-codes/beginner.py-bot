...
class Google(commands.Cog):
	...
	async def google(self, ctx, *, query):
		...
		embedded = discord.Embed(description=f"*{query}*\n\n[{results[0]['title']}]({results[0]['link']})\n[{results[1]['title']}]({results[1]['link']})\n[{results[2]['title']}]({results[2]['link']})\nIf you don't like these sites, you can check the other results here:\n[Google search for {query}]({url})", color=self.colors[randint(0,3)])
		embedded.set_author(name="Here are 3 Google results for", icon_url="https://cdn.discordapp.com/icons/644299523686006834/e69f6d4231a6e58eed5884625c4b4931.png")
